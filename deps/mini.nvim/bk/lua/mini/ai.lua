...
MiniAi.move_cursor = function(side, ai_type, id, opts)
    ...
    if
        (init_pos[1] == tobj_single[tobj_side].line)
        and (init_pos[2] == tobj_single[tobj_side].col - 1)
    then
...
MiniAi.gen_spec.argument = function(opts)
    ...
    local brackets, separators, exclude_regions =
        opts.brackets, opts.separators, opts.exclude_regions

    if type(opts.brackets) == "string" then
    ...
    res[2] = function(s, init)
        ...
        local seps = H.cache.argument_seps[res][s]
            or H.arg_get_separators(s, sep_pattern, exclude_regions)
...
MiniAi.gen_spec.pair = function(left, right, opts)
    ...
    if
        (opts.type == "balanced" or opts.type == "greedy")
        and not (left:len() == 1 and right:len() == 1)
    then
        local msg = string.format(
            [[Both `left` and `right` should be single character for `opts.type == '%s'`.]],
            opts.type
        )
        H.error(msg)
    end
...
MiniAi.gen_spec.treesitter = function(ai_captures, opts)
    ...
    return function(ai_type, _, _)
        ...
        local node_querier = (has_nvim_treesitter and opts.use_nvim_treesitter)
                and H.get_matched_nodes_plugin
        ...
        return vim.tbl_map(function(node)
            ...
            return {
                from = { line = line_from + 1, col = col_from + 1 },
                to = { line = line_to + 1, col = col_to },
            }
        end, matched_nodes)
    end
end
...
MiniAi.select_textobject = function(ai_type, id, opts)
    ...
    local vis_mode = opts.vis_mode
            and vim.api.nvim_replace_termcodes(opts.vis_mode, true, true, true)
        or prev_vis_mode
    ...
    pcall(function()
        ...
        if
            tobj_is_empty
            and opts.operator_pending
            and not (vim.v.operator == "c" or vim.v.operator == "d")
        then
...
H.make_textobject_table = function()
    ...
    local textobjects =
        vim.tbl_extend("force", H.builtin_textobjects, H.get_config().custom_textobjects or {})
    ...
    return setmetatable(textobjects, {
        __index = function(_, key)
            ...
            return {
                string.format("%s()()[^%s]-()%s+%%f[^%s]()", key_esc, key_esc, key_esc, key_esc),
            }
        end,
    })
end
...
H.is_region = function(x)
    ...
    local from_is_valid = type(x.from) == "table"
        and type(x.from.line) == "number"
        and type(x.from.col) == "number"
    -- Allow `to` to be `nil` to describe empty regions
    local to_is_valid = true
    if x.to ~= nil then
        to_is_valid = type(x.to) == "table"
            and type(x.to.line) == "number"
            and type(x.to.col) == "number"
    end
...
H.arg_get_separators = function(s, sep_pattern, exclude_regions)
    ...
    local res = vim.tbl_filter(function(x)
        return not H.is_point_inside_spans(x, forbidden)
    end, seps)
...
H.prepare_ai_captures = function(ai_captures)
    ...
    if
        not (
            type(ai_captures) == "table"
            and is_capture(ai_captures.a)
            and is_capture(ai_captures.i)
        )
    then
...
H.error_treesitter = function(failed_get, lang)
    ...
    local msg =
        string.format([[Can not get %s for buffer %d and language '%s'.]], failed_get, bufnr, lang)
    H.error(msg)
end
...
H.extract_span = function(s, extract_pattern, ai_type)
    ...
    if #positions == 2 then
        ...
        ai_spans = {
            a = H.new_span(positions[1], positions[4] - 1),
            i = H.new_span(positions[2], positions[3] - 1),
        }
    end
...
H.get_neighborhood = function(reference_region, n_neighbors)
    ...
    local from_line, to_line =
        reference_region.from.line, (reference_region.to or reference_region.from).line
...
H.get_visual_region = function()
    ...
    if
        res.from.line > res.to.line or (res.from.line == res.to.line and res.from.col > res.to.col)
    then
