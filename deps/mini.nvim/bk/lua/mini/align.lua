...
MiniAlign.align_strings = function(strings, opts, steps)
    ...
    if not H.is_parts(parts) then
        if H.can_be_parts(parts) then
            ...
            H.error(
                "Output of `split` step should be convertable to parts. See `:h MiniAlign.as_parts()`."
            )
        end
    end
        ...
        if mod == nil then
            ...
            if not ok then
                ...
                local msg = string.format(
                    "Modifier %s should be properly callable. Reason: %s",
                    vim.inspect(id),
                    out
                )
...
H.pad_functions = {
    ...
    center = function(x, n_spaces, no_trailing)
        ...
        return H.pad_functions.right(
            H.pad_functions.left(x, n_left, no_trailing),
            n_spaces - n_left,
            no_trailing
        )
    end,
...
H.default_action_justify = function(parts, opts)
    ...
    if not (H.is_justify_side(side) or H.is_array_of(side, H.is_justify_side)) then
        H.error(
            [[Option `justify_side` should be one of 'left', 'center', 'right', 'none', or array of those.]]
        )
    end
...
H.default_action_merge = function(parts, opts)
    ...
    return vim.tbl_map(function(row)
        local row_no_empty = vim.tbl_filter(function(s)
            return s ~= ""
        end, row)
...
H.region_get_text = function(region, mode)
    ...
    if mode == "block" then
        ...
        return vim.tbl_map(
            ...
            function(l)
                return vim.fn.strcharpart(l, left_virtcol - 1, n_cols)
            end,
    ...
    if mode == "block" then
        ...
        for i, l in ipairs(lines) do
            ...
            if left_virtcol <= n_virtcols then
                ...
                local line_left_col, line_right_col =
                    left_virtcol, math.min(right_virtcol, n_virtcols)

                -- Convert back to byte columns (columns are end-exclusive)
                local start_col, end_col =
                    vim.fn.byteidx(l, line_left_col - 1), vim.fn.byteidx(l, line_right_col)
...
H.region_virtcols = function(region)
    ...
    local left_virtcol, right_virtcol =
        math.min(from_virtcol, to_virtcol), math.max(from_virtcol, to_virtcol)
...
H.error = function(msg)
    error(string.format("(mini.align) %s", msg), 0)
end
