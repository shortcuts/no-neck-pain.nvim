...
H.validate_base16_palette = function(x, x_name)
    ...
    for _, color_name in pairs(H.base16_names) do
        ...
        if c == nil then
            local msg =
                string.format("(mini.base16) `%s` does not have value %s.", x_name, color_name)
            error(msg)
        end
...
H.validate_use_cterm = function(x, x_name)
    ...
    if type(x) ~= "table" then
        local msg = string.format(
            "(mini.base16) `%s` should be boolean or table with cterm colors.",
            x_name
        )
        error(msg)
    end
    ...
    for _, color_name in pairs(H.base16_names) do
        ...
        if not (type(c) == "number" and 0 <= c and c <= 255) then
            local msg =
                string.format("(mini.base16) `%s.%s` is not a cterm color.", x_name, color_name)
            error(msg)
        end
    end
...
H.validate_hex = function(x, x_name)
    ...
    if not is_hex then
        local msg =
            string.format('(mini.base16) `%s` is not a HEX color (string "#RRGGBB").', x_name)
        error(msg)
    end
