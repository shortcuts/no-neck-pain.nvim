...
MiniCompletion.setup = function(config)
    ...
    -- Create highlighting
    vim.api.nvim_exec(
        "hi default MiniCompletionActiveParameter term=underline cterm=underline gui=underline",
        false
    )
end
...
MiniCompletion.auto_completion = function()
    ...
    H.completion.timer:start(
        H.get_config().delay.completion,
        0,
        vim.schedule_wrap(H.trigger_twostep)
    )
end
...
H.setup_config = function(config)
    ...
    vim.validate({
        ...
        ["window_dimensions.signature.height"] = {
            config.window_dimensions.signature.height,
            "number",
        },
        ["window_dimensions.signature.width"] = {
            config.window_dimensions.signature.width,
            "number",
        },
    })
...
H.is_lsp_trigger = function(char, type)
    ...
    for _, client in pairs(vim.lsp.buf_get_clients()) do
        triggers =
            H.table_get(client, { "server_capabilities", providers[type], "triggerCharacters" })
...
H.show_info_window = function()
    ...
    vim.lsp.util.stylize_markdown(
        H.info.bufnr,
        lines,
        { wrap_at = H.get_config().window_dimensions.info.width }
    )
...
H.info_window_lines = function(info_id)
    ...
    local lsp_completion_item =
        H.table_get(completed_item, { "user_data", "nvim", "lsp", "completion_item" })
...
H.info_window_options = function()
    ...
    local info_height, info_width = H.floating_dimensions(
        lines,
        config.window_dimensions.info.height,
        config.window_dimensions.info.width
    )
    ...
    if space < info_width then
        info_height, info_width =
            H.floating_dimensions(lines, config.window_dimensions.info.height, space)
    end
...
H.signature_window_opts = function()
    ...
    if space < height then
        height, width =
            H.floating_dimensions(lines, space, config.window_dimensions.signature.width)
    end
