...
MiniIndentscope.auto_draw = function(opts)
    ...
    if
        opts.lazy
        and H.current.draw_status ~= "none"
        and H.scope_is_equal(scope, H.current.scope)
    then
        return
    end
...
MiniIndentscope.draw = function(scope, opts)
    ...
    local draw_opts =
        vim.tbl_deep_extend("force", { animation_fun = H.get_config().draw.animation }, opts or {})
...
H.border_from_body = {
    ...
    ["both"] = function(body, opts)
        return {
            ...
            indent = math.max(
                H.get_line_indent(body.top - 1, opts),
                H.get_line_indent(body.bottom + 1, opts)
            ),
        }
    end,
}
...
H.border_correctors = {
    ...
    ["top"] = function(line, opts)
        local cur_indent, next_indent =
            H.get_line_indent(line, opts), H.get_line_indent(line + 1, opts)
        return (cur_indent < next_indent) and (line + 1) or line
    end,
    ["bottom"] = function(line, opts)
        local prev_indent, cur_indent =
            H.get_line_indent(line - 1, opts), H.get_line_indent(line, opts)
        return (cur_indent < prev_indent) and (line - 1) or line
    end,
    ["both"] = function(line, opts)
        local prev_indent, cur_indent, next_indent =
            H.get_line_indent(line - 1, opts),
            H.get_line_indent(line, opts),
            H.get_line_indent(line + 1, opts)
...
H.setup_config = function(config)
    ...
    vim.validate({
        ...
        ["mappings.object_scope_with_border"] = {
            config.mappings.object_scope_with_border,
            "string",
        },
...
H.get_config = function(config)
    return vim.tbl_deep_extend(
        "force",
        MiniIndentscope.config,
        vim.b.miniindentscope_config or {},
        config or {}
    )
end
...
H.scope_has_intersect = function(scope_1, scope_2)
    ...
    if
        (scope_1.buf_id ~= scope_2.buf_id)
        or (H.scope_get_draw_indent(scope_1) ~= H.scope_get_draw_indent(scope_2))
    then
        return false
    end
  ...
  local arith_power_sum = ({
    ...
    local make_delta = function(n_steps, is_in_out)
        ...
        if is_in_out then
            ...
            total_parts = 2 * arith_power_sum(math.ceil(0.5 * n_steps))
                - (n_steps % 2 == 1 and 1 or 0)
        else
