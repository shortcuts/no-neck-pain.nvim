...
MiniJump2d.default_spotter = (function()
    ...
    return function(line_num, args)
        ...
        local res_2 = H.merge_unique(
            alphanum_before_punct(line_num, args),
            alphanum_after_punct(line_num, args)
        )
...
H.spots_to_extmarks = function(spots)
    ...
    local cur_col = col
    for _, s in ipairs(spots) do
        local is_within_same_extmark = s.buf_id == buf_id
            and s.line == (line + 1)
            and s.column == (cur_col + 1)

        if not is_within_same_extmark then
            table.insert(
                res,
                { buf_id = buf_id, col = col, line = line, text = table.concat(extmark_chars) }
            )
