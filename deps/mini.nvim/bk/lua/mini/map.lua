...
MiniMap.encode_strings = function(strings, opts)
    ...
    opts = vim.tbl_deep_extend(
        "force",
        {
            n_rows = math.huge,
            n_cols = math.huge,
            symbols = H.get_config().symbols.encode or H.default_symbols,
        },
        opts or {}
    )
...
MiniMap.refresh = function(opts, parts)
    ...
    -- Normalize input
    opts = H.normalize_opts(opts)
    parts = vim.tbl_deep_extend(
        "force",
        { integrations = true, lines = true, scrollbar = true },
        parts or {}
    )

    -- Update current data
    H.cache.scrollbar_data.offset = math.max(
...
MiniMap.toggle_focus = function(use_previous_cursor)
    if use_previous_cursor == nil then
        use_previous_cursor = false
    end
        ...
        for _, hunk in ipairs(hunks) do
            ...
            for i = 1, n_lines do
                ...
                if hl_group ~= nil then
                    table.insert(line_hl, { line = from_line + i - 1, hl_group = hl_group })
                end
            end
        end
...
H.mask_from_strings = function(strings, _)
    ...
    for i, s in ipairs(strings) do
        ...
        s_ext:gsub("()%s", function(j)
            mask_row[vim.str_utfindex(s_ext, j)] = false
        end)
        res[i] = mask_row
    end
...
H.mask_to_symbols = function(mask, opts)
    ...
    local symbols_n_rows, symbols_n_cols =
        math.ceil(#mask / row_resol), math.ceil(#mask[1] / col_resol)
...
H.is_valid_opts = function(x, x_name)
    ...
    local ok_integrations, msg_integrations =
        H.is_valid_config_integrations(x.integrations, x_name .. ".integrations")
...
H.create_map_buffer = function()
    ...
    vim.api.nvim_buf_set_keymap(
        ...
        "n",
        "<CR>",
        "<Cmd>lua MiniMap.toggle_focus(false)<CR>",
        { noremap = false }
    )
    vim.api.nvim_buf_set_keymap(
        buf_id,
        "n",
        "<Esc>",
        "<Cmd>lua MiniMap.toggle_focus(true)<CR>",
        { noremap = false }
    )
    ...
    local cursormoved_cmd =
        string.format("au CursorMoved <buffer=%d> lua MiniMap.track_map_cursor()", buf_id)
...
H.update_window_opts = function()
    ...
    -- 'winblend'
    vim.api.nvim_win_call(win_id, function()
        vim.cmd("setlocal winblend=" .. opts.window.winblend)
    end)
end
...
H.update_map_scrollbar = function()
    ...
    if not (view.from_line == cur_view.from_line and view.to_line == cur_view.to_line) then
        ...
        local extmark_opts = {
            virt_text = { { symbols.scroll_view, "MiniMapSymbolView" } },
    ...
    if scroll_line ~= cur_line then
        ...
        local extmark_opts = {
            virt_text = { { symbols.scroll_line, "MiniMapSymbolLine" } },
            virt_text_pos = "overlay",
            priority = 11,
        }
...
H.error = function(msg)
    error(string.format("(mini.map) %s", msg), 0)
end
...
H.add_line_hl = function(buf_id, ns_id, hl_group, line)
    pcall(
        ...
        ns_id,
        hl_group,
        { line, H.cache.scrollbar_data.offset },
        { line, -1 }
    )
end
