...
MiniStarter.sections.sessions = function(n, recent)
    ...
    return function()
        if _G.MiniSessions == nil then
            return {
                { name = [['mini.sessions' is not set up]], action = "", section = "Sessions" },
            }
        end
...
MiniStarter.on_cursormoved = function(buf_id)
    buf_id = buf_id or vim.api.nvim_get_current_buf()
    if not H.validate_starter_buf_id(buf_id, "on_cursormoved()") then
        return
    end
    H.position_cursor_on_current_item(buf_id)
end
...
H.default_content_hooks =
    { MiniStarter.gen_hook.adding_bullet(), MiniStarter.gen_hook.aligning("center", "center") }
...
H.content_add_items = function(content, items)
    ...
    for _, item in ipairs(items) do
        ...
        if cur_section ~= item.section then
            ...
            H.content_add_line(
                content,
                { H.content_unit(item.section, "section", "MiniStarterSection") }
            )
            cur_section = item.section
        end
        ...
        H.content_add_line(
            content,
            { H.content_unit(item.name, "item", "MiniStarterItem", { item = item }) }
        )
    end
end
...
H.make_query = function(buf_id, query, echo_msg)
    ...
    if n_active == 0 and query ~= "" then
        H.message(
            ("Query %s results into no active items. Current query: %s"):format(
                vim.inspect(query),
                data.query
            )
        )
...
H.validate_starter_buf_id = function(buf_id, fun_name, severity)
    ...
    local msg =
        string.format("`buf_id` in `%s` is not an identifier of valid Starter buffer.", fun_name)
