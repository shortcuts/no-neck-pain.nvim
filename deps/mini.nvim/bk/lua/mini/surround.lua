...
MiniSurround.add = function(mode)
    ...
    H.region_replace(
        { from = { line = marks.second.line, col = marks.second.col + 1 } },
        surr_info.right
    )
...
MiniSurround.update_n_lines = function()
    ...
    local n_lines =
        MiniSurround.user_input("New number of neighbor lines", MiniSurround.config.n_lines)
...
MiniSurround.gen_spec.input.treesitter = function(captures, opts)
    ...
    return function()
        ...
        local node_pair_querier = (has_nvim_treesitter and opts.use_nvim_treesitter)
                and H.get_matched_node_pairs_plugin
        ...
        return vim.tbl_map(function(node_pair)
            ...
            local left_from_line, left_from_col, right_to_line, right_to_col =
                node_pair.outer:range()
            ...
            if node_pair.inner == nil then
                ...
                local left_to_line, left_to_col, right_from_line, right_from_col =
                    node_pair.inner:range()
...
H.make_surrounding_table = function()
    ...
    local surroundings = vim.tbl_deep_extend(
        "force",
        H.builtin_surroundings,
        H.get_config().custom_surroundings or {}
    )
...
H.find_surrounding_region_pair = function(surr_spec, opts)
    ...
    return {
        left = neigh.span_to_region(final_spans.left),
        right = neigh.span_to_region(final_spans.right),
    }
end
...
H.prepare_captures = function(captures)
    ...
    if
        not (
            type(captures) == "table"
            and is_capture(captures.outer)
            and is_capture(captures.inner)
        )
    then
        H.error(
            "Wrong format for `captures`. See `MiniSurround.gen_spec.input.treesitter()` for details."
        )
    end
...
H.get_matched_node_pairs_plugin = function(captures)
    ...
    if parser then
        parser:for_each_tree(function(tree, lang_tree)
            ...
            local lang_matches =
                ts_queries.get_capture_matches(0, captures.outer, "textobjects", tree:root(), lang)
    ...
    return vim.tbl_map(
        function(match)
            ...
            local matches_inner = ts_queries.get_capture_matches(
                0,
                captures.inner,
                "textobjects",
                node_outer,
...
H.find_best_match = function(neighborhood, surr_spec, reference_span, opts)
    ...
    if H.is_region_pair_array(surr_spec) then
        ...
        for _, region_pair in ipairs(surr_spec) do
            ...
            local outer_region = {
                from = region_pair.left.from,
                to = region_pair.right.to or region_pair.right.from,
            }
...
H.extract_surr_spans = function(s, extract_pattern)
    ...
    return {
        left = H.new_span(positions[1], positions[2] - 1),
        right = H.new_span(positions[3], positions[4] - 1),
    }
end
