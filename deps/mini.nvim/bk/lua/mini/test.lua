...
MiniTest.collect = function(opts)
    ...
    for _, file in ipairs(opts.find_files()) do
        ...
        if not ok then
            local msg =
                string.format("Sourcing %s resulted into following error: %s", vim.inspect(file), t)
            H.error(msg)
        end
        local is_output_correct = (opts.emulate_busted and vim.tbl_count(set[file]) > 0)
            or H.is_instance(t, "testset")
...
MiniTest.execute = function(cases, opts)
    ...
    local reporter = opts.reporter
        or (H.is_headless and MiniTest.gen_reporter.stdout() or MiniTest.gen_reporter.buffer())
    ...
    for case_num, cur_case in ipairs(cases) do
        ...
        vim.schedule(function()
            MiniTest.current.case = cur_case
        end)
        ...
        schedule_step(function()
            cur_case.test(unpack(cur_case.args))
        end, "Executing test")
        ...
        -- Finalize state
        schedule_step(nil, function()
            return H.case_final_state(cur_case)
        end)
    end
...
MiniTest.expect.error = function(f, pattern, ...)
    ...
    local matching_pattern = pattern == nil and ""
        or (" matching pattern %s"):format(vim.inspect(pattern))
...
MiniTest.new_child_neovim = function()
    ...
    child.start = function(args, opts)
        ...
        job.stdin, job.stdout, job.stderr =
            vim.loop.new_pipe(false), vim.loop.new_pipe(false), vim.loop.new_pipe(false)
    ...
    for _, key in ipairs(node_keys) do
        ...
        for _, args in ipairs(parametrize) do
            ...
            elseif H.is_instance(node, "testset") then
                local nest_testcase_arr, nest_hooks_once_arr =
                    H.set_to_testcases(node, cur_template, hooks_once)
...
H.buffer_reporter.set_lines = function(buf_id, lines, start, finish)
    ...
    for i, l in ipairs(lines) do
        ...
        local new_l = l:gsub("()(\27%[.-m)(.-)\27%[0m", function(...)
            ...
            table.insert(
                hl_ranges,
                {
                    hl = H.hl_groups[dots[2]],
                    line = start + i - 1,
                    left = left - 1,
                    right = left + dots[3]:len() - 1,
                }
            )
    ...
    for _, hl_data in ipairs(hl_ranges) do
        vim.highlight.range(
            ...
            ns_id,
            hl_data.hl,
            { hl_data.line, hl_data.left },
            { hl_data.line, hl_data.right }
        )
    end
end
...
H.screenshot_compare = function(screen_ref, screen_obs)
    local compare = function(x, y, desc)
        if x ~= y then
            return false,
                ("Different %s. Reference: %s. Observed: %s."):format(
                    desc,
                    vim.inspect(x),
                    vim.inspect(y)
                )
...
H.screenshot_read = function(path)
    ...
    local n = 0.5 * (#lines - 3)
    local text_lines, attr_lines =
        vim.list_slice(lines, 2, n + 1), vim.list_slice(lines, n + 4, 2 * n + 3)
