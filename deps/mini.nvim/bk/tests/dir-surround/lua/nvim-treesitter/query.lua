local new_match = function(range, id)
    return {
        node = {
            ...
            start = function(_)
                return range[1], range[2], vim.fn.line2byte(range[1] + 1) + range[2] - 1
            end,

            -- Return end row, end col, and number of bytes from buffer start
            end_ = function(_)
                return range[3], range[4] - 1, vim.fn.line2byte(range[3] + 1) + range[4] - 2
            end,
        },
    }
end
...
local get_capture_matches = function(_, capture, _, node, _)
    ...
    return vim.tbl_filter(function(m)
        return vim.tbl_contains(valid_ids, m.node._id)
    end, all_matches)
end

return {
    get_capture_matches_recursively = get_capture_matches_recursively,
    get_capture_matches = get_capture_matches,
}
