...
local validate_next_region = function(keys, next_region)
    type_keys(keys)
    eq(
        { { child.fn.line("."), child.fn.col(".") }, { child.fn.line("v"), child.fn.col("v") } },
        next_region
    )
end
...
local mock_treesitter_builtin = function()
    child.cmd("source tests/dir-ai/mock-lua-treesitter.lua")
end

local mock_treesitter_plugin = function()
    child.cmd("set rtp+=tests/dir-ai")
end
...
T["setup()"]["creates `config` field"] = function()
    ...
    local expect_config = function(field, value)
        eq(child.lua_get("MiniAi.config." .. field), value)
    end
...
T["setup()"]["properly handles `config.mappings`"] = function()
    local has_map = function(lhs)
        return child.cmd_capture("xmap " .. lhs):find("MiniAi") ~= nil
    end
    eq(has_map("a"), true)
...
local find_textobject = function(...)
    return child.lua_get("MiniAi.find_textobject(...)", { ... })
end
...
local validate_find1d = function(line, column, args, expected)
    local new_expected
    if expected ~= nil then
        new_expected = { { 1, expected[1] }, { 1, expected[2] } }
    end
...
T["find_textobject()"]["respects `id` argument"] = function()
    validate_find1d("(aa[bb]cc)", 4, { "a", "]" }, { 4, 7 })
end
...
T["find_textobject()"]["respects `opts.reference_region`"] = function()
    ...
    local new_opts = function(from, to)
        return {
            reference_region = { from = { line = 1, col = from }, to = { line = 1, col = to } },
        }
    end
...
T["find_textobject()"]["respects `opts.search_method`"] = function()
    ...
    local new_opts = function(search_method)
        return { search_method = search_method }
    end
    ...
    expect.error(function()
        find_textobject("a", ")", { search_method = "aaa" })
    end, "one of")
end
...
T["find_textobject()"]["ensures that output is not covered by reference"] = function()
    ...
    eq(
        find_textobject(
            "i",
            ")",
            { reference_region = { from = { line = 1, col = 8 }, to = { line = 1, col = 9 } } }
        ),
...
T["find_textobject()"]["handles function as textobject spec"]["returns composed pattern"] =
    function()
...
T["find_textobject()"]["handles function as textobject spec"]["returns array of regions"] =
    function()
        -- Should take arguments from corresponding `find_textobject()` call
...
T["move_cursor()"]["works"] = function()
    validate_move1d("aa(bbb)", 4, { "left", "a", ")" }, 2)
end
...
T["move_cursor()"]["respects `side` argument"] = function()
    ...
    expect.error(function()
        child.lua([[MiniAi.move_cursor('leftright', 'a', ')')]])
    end, vim.pesc([[(mini.ai) `side` should be one of 'left' or 'right'.]]))
end
...
T["move_cursor()"]["respects `id` argument"] = function()
    validate_move1d("aa[bbb]", 4, { "left", "a", "]" }, 2)
end
...
T["move_cursor()"]["handles function as textobject spec"] = function()
    ...
    child.lua("_G.n = 0")
    child.lua(
        [[MiniAi.config.custom_textobjects = { c = function() _G.n = _G.n + 1; return { '()c()' } end }]]
    )
...
T["gen_spec"]["argument()"]["respects `opts.brackets`"] = function()
    child.lua(
        [[MiniAi.config.custom_textobjects = { A = MiniAi.gen_spec.argument({ brackets = { '%b()' } }) }]]
    )
...
T["gen_spec"]["function_call()"]["respects `opts.name_pattern`"] = function()
    child.lua(
        [[MiniAi.config.custom_textobjects = { F = MiniAi.gen_spec.function_call({ name_pattern = '[%w_]' }) }]]
    )
...
T["gen_spec"]["treesitter()"]["validates `ai_captures` argument"] = function()
    ...
    local validate = function(args)
        expect.error(function()
            child.lua([[MiniAi.gen_spec.treesitter(...)]], { args })
        end, "ai_captures")
    end
...
T["gen_spec"]["treesitter()"]["validates builtin treesitter presence"] = function()
    ...
    expect.error(function()
        child.lua([[MiniAi.find_textobject('a', 'F')]])
    end, vim.pesc([[(mini.ai) Can not get query for buffer 1 and language 'lua'.]]))

    -- Parser
    child.bo.filetype = "aaa"
    expect.error(function()
        child.lua([[MiniAi.find_textobject('a', 'F')]])
    end, vim.pesc([[(mini.ai) Can not get parser for buffer 1 and language 'aaa'.]]))
end
...
local validate_select = function(lines, cursor, args, expected)
    ...
    local expected_from = type(expected[1]) == "number" and expected[1]
        or { expected[1][1], expected[1][2] - 1 }
    local expected_to = type(expected[2]) == "number" and expected[2]
        or { expected[2][1], expected[2][2] - 1 }
...
T["select_textobject()"]["works"] = function()
...
T["select_textobject()"]["works with empty region"] = function()
    validate_select1d("a()", 0, { "i", ")" }, { 3, 3 })
end
...
T["expr_textobject()"]["is present"] = function()
    eq(child.lua_get("type(MiniAi.expr_textobject)"), "function")
end

-- Actual testing is done in 'Integration tests'
T["expr_motion()"] = new_set()

T["expr_motion()"]["is present"] = function()
    eq(child.lua_get("type(MiniAi.expr_motion)"), "function")
end
...
T["Search method"]['works with "cover"'] = function()
    ...
    local validate_n_times = function(lines, cursor, n_times, expected)
        validate_find(
            lines,
            cursor,
            { "a", ")", { search_method = "cover", n_times = n_times } },
            expected
        )
    end
...
T["Search method"]['works with "cover_or_next"'] = function()
    ...
    local validate_n_times = function(lines, cursor, n_times, expected)
        validate_find(
            lines,
            cursor,
            { "a", ")", { search_method = "cover_or_next", n_times = n_times } },
            expected
        )
    end
...
T["Search method"]['works with "cover_or_prev"'] = function()
    ...
    local validate_n_times = function(lines, cursor, n_times, expected)
        validate_find(
            lines,
            cursor,
            { "a", ")", { search_method = "cover_or_prev", n_times = n_times } },
            expected
        )
    end
...
T["Search method"]['works with "cover_or_nearest"'] = function()
    ...
    local validate1d = function(line, column, expected)
        validate_find1d(
            line,
            column,
            { "a", ")", { search_method = "cover_or_nearest" } },
            expected
        )
    end
    ...
    local validate_distance = function(region_cols, expected)
        local reference_region =
            { from = { line = 1, col = region_cols[1] }, to = { line = 1, col = region_cols[2] } }
        validate_find1d(
            "(aa)bbb(cc)",
            0,
            {
                "a",
                ")",
                { search_method = "cover_or_nearest", reference_region = reference_region },
            },
            expected
        )
    end
    ...
    local validate_n_times = function(lines, cursor, n_times, expected)
        validate_find(
            lines,
            cursor,
            { "a", ")", { search_method = "cover_or_nearest", n_times = n_times } },
            expected
        )
    end
...
T["Search method"]['works with "next"'] = function()
    ...
    local validate_n_times = function(lines, cursor, n_times, expected)
        validate_find(
            lines,
            cursor,
            { "a", ")", { search_method = "next", n_times = n_times } },
            expected
        )
    end
...
T["Search method"]['works with "prev"'] = function()
    ...
    local validate_n_times = function(lines, cursor, n_times, expected)
        validate_find(
            lines,
            cursor,
            { "a", ")", { search_method = "prev", n_times = n_times } },
            expected
        )
    end
...
T["Search method"]['works with "nearest"'] = function()
    ...
    local validate_n_times = function(lines, cursor, n_times, expected)
        validate_find(
            lines,
            cursor,
            { "a", ")", { search_method = "nearest", n_times = n_times } },
            expected
        )
    end
...
local validate_no_tobj1d = function(line, column, keys)
    validate_no_tobj({ line }, { 1, column }, keys)
end
...
T["Textobject"]["falls back in case of absent textobject id"] = function()
    ...
    local map = function(mode, lhs)
        child.api.nvim_set_keymap(
            mode,
            lhs,
            "<Cmd>lua vim.api.nvim_win_set_cursor(0, { 1, 5 })<CR>",
            { noremap = true }
        )
    end
...
T["Textobject"]["prompts helper message after one idle second"] = new_set(
    { parametrize = { { "a" }, { "i" } } },
    {
        test = function(key)
            child.set_size(5, 70)
...
T["Builtin"]["Bracket"] =
    new_set({ parametrize = vim.tbl_map(function(x)
        return { x }
    end, vim.tbl_keys(brackets)) })
...
T["Builtin"]["Bracket"]["handles inner whitespace"] = function(key)
    ...
    validate_tobj1d(
        ...
        0,
        { "i", key },
        side == "open" and { 2, 3 } or { 2, 4 }
    )
    validate_tobj1d(
        left .. " aa" .. right,
        0,
        { "i", key },
        side == "open" and { 3, 4 } or { 2, 4 }
    )
    validate_tobj1d(
        left .. " aa " .. right,
        0,
        { "i", key },
        side == "open" and { 3, 4 } or { 2, 5 }
    )
...
T["Builtin"]["Quote"]["is balanced"] = function(key)
    ...
    -- Reference: {'(', 'aa(bb)cc(dd)', ')'}
    local lines, cursor =
        { key, string.format("aa%sbb%scc%sdd%s", key, key, key, key), key }, { 2, 0 }
...
T["Builtin"]["User prompt"]["works with empty region"] = function()
    validate_tobj1d("_eo", 0, "i?e<CR>o<CR>", { 3, 3 })
end
...
local set_custom_tobj = function(tbl)
    child.lua("MiniAi.config.custom_textobjects = " .. vim.inspect(tbl))
end
...
T["Custom textobject"]["handles function as textobject spec"] = function()
  ...
  eq(
    ...
    local lines =
        { string.rep("a", 80), string.rep("b", 81), string.rep("c", 80), string.rep("d", 81) }
...
T["Custom textobject"]["documented examples"]["function call with name from user input"] =
    function()
...
T["Custom textobject"]["documented examples"]["balanced parenthesis with big enough width"] =
    function()
