...
local validate_step = function(var_name, step_name)
    ...
    if step_name ~= nil then
        eq(child.lua_get(("%s.name"):format(var_name)), step_name)
    end
...
local get_mode = function()
    return child.api.nvim_get_mode()["mode"]
end
...
T["setup()"]["creates `config` field"] = function()
    ...
    local expect_config = function(field, value)
        eq(child.lua_get("MiniAlign.config." .. field), value)
    end
...
T["setup()"]["validates `config` argument"] = function()
    ...
    expect_config_error(
        { mappings = { start_with_preview = 1 } },
        "mappings.start_with_preview",
        "string"
    )
...
T["setup()"]["properly handles `config.mappings`"] = function()
    local has_map = function(lhs)
        return child.cmd_capture("xmap " .. lhs):find("MiniAlign") ~= nil
    end
    eq(has_map("ga"), true)
...
local validate_align_strings = function(input_strings, opts, ref_strings, steps)
    local output =
        child.lua_get("MiniAlign.align_strings(...)", { input_strings, opts or {}, steps or {} })
...
T["align_strings()"]["validates `strings` argument"] = function()
    expect.error(function()
        child.lua([[MiniAlign.align_strings({'a', 1})]])
    end, "string")
    expect.error(function()
...
T["align_strings()"]["respects `opts` argument"] = function()
    ...
    eq(
        child.lua_get([[MiniAlign.align_strings({ 'a=b', 'aa=b' }, { split_pattern = '=' }, {})]]),
        { "xxx", "aa=b" }
    )
end
...
T["align_strings()"]["validates `steps` argument"] = function()
    ...
    local validate = function(steps_str, error_pattern)
        expect.error(function()
            local cmd =
                string.format([[MiniAlign.align_strings({'a=b', 'aa=b'}, {}, %s)]], steps_str)
            child.lua(cmd)
        end, error_pattern)
    end
...
T["align_strings()"]["respects `steps.pre_split` argument"] = function()
    ...
    cmd = string.format(
        [[MiniAlign.align_strings({ 'aaa=b', 'aa=b' }, {}, { pre_split = { %s } })]],
        step_str
    )
    ...
    cmd = string.format(
        [[MiniAlign.align_strings({ 'a=b', 'aa=b' }, {}, { pre_split = { %s } })]],
        step_str
    )
    expect.error(
        child.lua,
        "Step `tmp` of `pre_split` should preserve structure of `strings`.",
        cmd
    )

    -- Uses `MiniAlign.config.steps` as default
    set_config_steps({
        pre_split = [[{ MiniAlign.new_step('tmp', function(strings) strings[1] = 'a=b' end) }]],
    })
    ...
    cmd = string.format(
        [[MiniAlign.align_strings({ 'aaa=b', 'aa=b' }, { tmp = 'xxx' }, { pre_split = { %s } })]],
        step_str
    )
...
T["align_strings()"]["respects `steps.split` argument"] = function()
    ...
    step_str =
        [[MiniAlign.new_step('tmp', function(strings) return { { 'a', 'b' }, {'aa', 'b'} } end)]]
    cmd =
        string.format([[MiniAlign.align_strings({ 'a=b', 'aa=b' }, {}, { split = %s })]], step_str)
    ...
    step_str =
        [[MiniAlign.new_step('tmp', function(strings) return { { 'a', 1 }, {'aa', 'b'} } end)]]
    cmd =
        string.format([[MiniAlign.align_strings({ 'a=b', 'aa=b' }, {}, { split = %s })]], step_str)
    ...
    -- Is called with `opts`
    step_str =
        [[MiniAlign.new_step('tmp', function(strings, opts) return MiniAlign.as_parts(opts.tmp) end)]]
    cmd = string.format(
        [[MiniAlign.align_strings({ 'a=b', 'aa=b' }, { tmp = { { 'xxx' } } }, { split = %s })]],
        step_str
    )
...
T["align_strings()"]["respects `steps.pre_justify` argument"] = function()
    ...
    cmd = string.format(
        [[MiniAlign.align_strings({ 'aaa=b', 'aa=b' }, {}, { pre_justify = { %s } })]],
        step_str
    )
    ...
    cmd = string.format(
        [[MiniAlign.align_strings({ 'a=b', 'aa=b' }, {}, { pre_justify = { %s } })]],
        step_str
    )
    expect.error(
        child.lua,
        vim.pesc(
            "Step `tmp` of `pre_justify` should preserve structure of `parts`. See `:h MiniAlign.as_parts()`."
        ),
        cmd
    )
    ...
    -- Uses `MiniAlign.config.steps` as default
    set_config_steps({
        pre_justify = [[{ MiniAlign.new_step('tmp', function(parts) parts[1][1] = 'xxx' end) }]],
    })
    ...
    cmd = string.format(
        [[MiniAlign.align_strings({ 'aaa=b', 'aa=b' }, { tmp = 'xxx' }, { pre_justify = { %s } })]],
        step_str
    )
    eq(child.lua_get(cmd), { "xxx=b", "aa =b" })
end
...
T["align_strings()"]["respects `steps.justify` argument"] = function()
    ...
    step_str = [[MiniAlign.new_step('tmp', function(parts) parts[1][1] = 'xxx' end)]]
    cmd = string.format(
        [[MiniAlign.align_strings({ 'a=b', 'aa=b' }, {}, { justify = %s })]],
        step_str
    )
    eq(child.lua_get(cmd), { "xxx=b", "aa=b" })
    ...
    expect.error(
        child.lua,
        vim.pesc(
            "Step `tmp` of `justify` should preserve structure of `parts`. See `:h MiniAlign.as_parts()`."
        ),
        cmd
    )
    ...
    step_str = [[MiniAlign.new_step('tmp', function(parts, opts) parts[1][1] = opts.tmp end)]]
    cmd = string.format(
        [[MiniAlign.align_strings({ 'a=b', 'aa=b' }, { tmp = 'xxx' }, { justify = %s })]],
        step_str
    )
    eq(child.lua_get(cmd), { "xxx=b", "aa=b" })
end
...
T["align_strings()"]["respects `steps.pre_merge` argument"] = function()
    ...
    step_str = [[MiniAlign.new_step('tmp', function(parts) parts[1][1] = 'xxx' end)]]
    cmd = string.format(
        [[MiniAlign.align_strings({ 'aaa=b', 'aa=b' }, {}, { pre_merge = { %s } })]],
        step_str
    )
    eq(child.lua_get(cmd), { "xxx=b", "aa =b" })
    ...
    step_str = [[MiniAlign.new_step('tmp', function(parts) parts[1][1] = 1 end)]]
    cmd = string.format(
        [[MiniAlign.align_strings({ 'a=b', 'aa=b' }, {}, { pre_merge = { %s } })]],
        step_str
    )
    expect.error(
        child.lua,
        vim.pesc(
            "Step `tmp` of `pre_merge` should preserve structure of `parts`. See `:h MiniAlign.as_parts()`."
        ),
        cmd
    )
    ...
    -- Uses `MiniAlign.config.steps` as default
    set_config_steps({
        pre_merge = [[{ MiniAlign.new_step('tmp', function(parts) parts[1][1] = 'xxx' end) }]],
    })
    validate_align_strings({ "aaa=b", "aa=b" }, { split_pattern = "=" }, { "xxx=b", "aa =b" })
    ...
    step_str = [[MiniAlign.new_step('tmp', function(parts, opts) parts[1][1] = opts.tmp end)]]
    cmd = string.format(
        [[MiniAlign.align_strings({ 'aaa=b', 'aa=b' }, { tmp = 'xxx' }, { pre_merge = { %s } })]],
        step_str
    )
    eq(child.lua_get(cmd), { "xxx=b", "aa =b" })
end
...
T["align_strings()"]["respects `steps.merge` argument"] = function()
    ...
    cmd =
        string.format([[MiniAlign.align_strings({ 'a=b', 'aa=b' }, {}, { merge = %s })]], step_str)
    eq(child.lua_get(cmd), { "xxx" })
    ...
    cmd = string.format(
        [[MiniAlign.align_strings({ 'a=b', 'aa=b' }, { tmp = 'xxx' }, { merge = %s })]],
        step_str
    )
    eq(child.lua_get(cmd), { "xxx" })
end
...
T["align_strings()"]["does not affect input array"] = function()
    ...
    child.lua(
        [[MiniAlign.align_strings(strings, { split_pattern = '=' }, { pre_split = pre_split })]]
    )
...
T["as_parts()"]["validates arguments"] = function()
    local validate = function(input_str, err_pattern)
        expect.error(function()
            child.lua("MiniAlign.as_parts(" .. input_str .. ")")
        end, err_pattern)
    end
...
T["as_parts()"]["`apply_inplace()` method"] = function()
    child.lua([[parts = MiniAlign.as_parts({ { 'a', 'b' }, { 'c' } })]])
    child.lua(
        "new_parts = parts.apply_inplace(function(x, data) return x .. data.row .. data.col end)"
    )
...
T["as_parts()"]["`trim()` method"]["validates arguments"] = function()
    ...
    expect.error(function()
        child.lua([[parts.trim(1)]])
    end, err_pattern)
    expect.error(function()
        child.lua([[parts.trim('a')]])
    end, err_pattern)

    -- `indent`
    err_pattern = '`indent` should be one of "high", "keep", "low", "remove"'
    expect.error(function()
        child.lua([[parts.trim('both', 1)]])
    end, err_pattern)
    expect.error(function()
        child.lua([[parts.trim('both', 'a')]])
    end, err_pattern)
end
...
T["new_step()"]["validates arguments"] = function()
    local validate = function(args_str, err_pattern)
        expect.error(function()
            child.lua("MiniAlign.new_step(" .. args_str .. ")")
        end, err_pattern)
    end
...
T["gen_step"]["default_split()"] = new_set({
    hooks = {
        pre_case = function()
            set_config_steps({ split = [[MiniAlign.gen_step.default_split('test')]] })
        end,
    },
})
...
T["gen_step"]["default_split()"]["verifies relevant options"] = function()
    expect.error(function()
        child.lua([[MiniAlign.align_strings({ 'a' }, { split_pattern = 1 }, {})]])
    end, "Option `split_pattern`.*string or array of strings")
    expect.error(function()
        child.lua([[MiniAlign.align_strings({ 'a' }, { split_exclude_patterns = 1 }, {})]])
    end, "Option `split_exclude_patterns`.*array of strings")
end
...
T["gen_step"]["default_split()"]["verifies bad split pattern"] = function()
    expect.error(function()
        child.lua([[MiniAlign.align_strings({ 'a ' }, { split_pattern = '%f[%s]' })]])
    end, vim.pesc('(mini.align) Pattern "%f[%s]" can not advance search.'))
end
...
T["gen_step"]["default_split()"]["works with special exclude patterns"] = function()
    ...
    validate_align_strings(
        lines,
        { split_pattern = "=", split_exclude_patterns = { "^a.*" } },
        output_lines
    )
...
T["gen_step"]["default_split()"]["matches inside forbidden spans do not affect split pattern recycling"] =
    function()
        validate_align_strings(
...
T["gen_step"]["default_justify()"] = new_set({
    hooks = {
        pre_case = function()
            set_config_steps({ justify = [[MiniAlign.gen_step.default_justify('test')]] })
        end,
    },
})
...
T["gen_step"]["default_justify()"]["verifies relevant options"] = function()
    expect.error(function()
        child.lua([[MiniAlign.align_strings({ 'a' }, { justify_side = 1 }, {})]])
    end, "Option `justify_side`.*one of.*or array")
end
...
T["gen_step"]["default_justify()"]["last row element width is ignored for left justify side"] =
    function()
...
T["gen_step"]["default_merge()"] = new_set({
    hooks = {
        pre_case = function()
            set_config_steps({ merge = [[MiniAlign.gen_step.default_merge('test')]] })
        end,
    },
})
...
T["gen_step"]["default_merge()"]["verifies relevant options"] = function()
    expect.error(function()
        child.lua([[MiniAlign.align_strings({ 'a' }, { merge_delimiter = 1 }, {})]])
    end, "Option `merge_delimiter`.*string or array of strings")
end
...
T["gen_step"]["filter()"]["validates input"] = function()
    expect.error(function()
        child.lua([[MiniAlign.gen_step.filter('(')]])
    end, [[%(mini%.align%) "%(" is not a valid filter expression]])
end
...
T["gen_step"]["filter()"]["allows special variables"] = function()
    ...
    local set = function(expr)
        set_config_steps({
            pre_justify = ([[{ MiniAlign.gen_step.filter(%s) }]]):format(vim.inspect(expr)),
        })
    end
...
T["gen_step"]["ignore_split()"]["validates input"] = function()
    expect.error(function()
        child.lua([[MiniAlign.gen_step.ignore_split('(')]])
    end, [[Argument `patterns`.*array of strings]])
    expect.error(function()
        child.lua([[MiniAlign.gen_step.ignore_split({}, 1)]])
    end, [[Argument `exclude_comment`.*boolean]])
end
...
T["gen_step"]["ignore_split()"]["respects `patterns` argument"] = function()
    ...
    child.o.commentstring = "# %s"
    set_config_steps({
        pre_split = [[{ MiniAlign.gen_step.ignore_split({ '*.-*', '".-"' }), test_step }]],
    })
...
T["gen_step"]["trim()"]["respects `direction` argument"] = function()
    ...
    local set = function(direction)
        set_config_steps({
            pre_justify = ([[{ MiniAlign.gen_step.trim(%s) }]]):format(vim.inspect(direction)),
        })
    end
...
T["gen_step"]["trim()"]["respects `indent` argument"] = function()
    ...
    local set = function(indent)
        set_config_steps({
            pre_justify = ([[{ MiniAlign.gen_step.trim('both', %s) }]]):format(vim.inspect(indent)),
        })
    end
...
T["Align"]["does not stop on error during modifier execution"] = function()
    ...
    expect.match(
        get_latest_message(),
        '^%(mini.align%) Modifier "e" should be properly callable%. Reason:'
    )
end
...
T["Modifiers"]["<BS>"]["works"] = new_set(
    { parametrize = { { "pre_split" }, { "pre_justify" }, { "pre_merge" } } },
    {
...
T["Modifiers"]["<equal sign>"] = function()
    validate_common_split({ "a=b", "aaa=bbb" }, "=")
end

T["Modifiers"]["<comma>"] = function()
    validate_common_split({ "a,b", "aaa,bbb" }, ",")
end

T["Modifiers"]["<space bar>"] = function()
    validate_common_split({ "  a  b", "    aaa    bbb", "a b" }, " ")
end
