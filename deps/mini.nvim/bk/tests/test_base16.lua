...
T["setup()"]["validates `config` argument"] = function()
    ...
    expect_config_error(
        { palette = minischeme_palette, use_cterm = "a" },
        "use_cterm",
        "boolean or table"
    )
    expect_config_error(
...
T["setup()"]["defines builtin highlight groups"] = function()
    ...
    validate_hl_group(
        "Error",
        ("ctermfg=15 ctermbg=9 guifg=%s guibg=%s"):format(p.base00, p.base08)
    )
...
T["setup()"]["defines highlight groups for terminal colors"] = function()
    ...
    vim.g.terminal_color_background = vim.o.background == "dark" and vim.g.terminal_color_0
        or vim.g.terminal_color_7
    vim.g.terminal_color_foreground = vim.o.background == "dark" and vim.g.terminal_color_5
...
T["setup()"]["respects `config.use_cterm`"] = function()
    ...
    validate_hl_group(
        "Normal",
        ("ctermfg=%s ctermbg=%s guifg=%s guibg=%s"):format(
            p_cterm.base05,
            p_cterm.base00,
            p.base05,
            p.base00
...
T["rgb_palette_to_cterm_palette()"]["works"] = function()
    eq(
        child.lua_get("MiniBase16.rgb_palette_to_cterm_palette(...)", { minischeme_palette }),
        minischeme_use_cterm
    )
end
