...
T["setup()"]["validates `config` argument"] = function()
    ...
    expect_config_error(
        { window_dimensions = { info = { height = "a" } } },
        "window_dimensions.info.height",
        "number"
    )
    expect_config_error(
        { window_dimensions = { info = { width = "a" } } },
        "window_dimensions.info.width",
        "number"
    )
    expect_config_error(
        { window_dimensions = { signature = "a" } },
        "window_dimensions.signature",
        "table"
    )
    expect_config_error(
    ...
    expect_config_error(
        { lsp_completion = { auto_setup = "a" } },
        "lsp_completion.auto_setup",
        "boolean"
    )
    expect_config_error(
        { lsp_completion = { process_items = "a" } },
...
T["Autocompletion"]["respects `config.lsp_completion.process_items`"] = function()
    ...
    child.lua(
        "vim.b.minicompletion_config = { lsp_completion = { process_items = _G.process_items_2 } }"
    )
...
T["Autocompletion"]["respects function `config.fallback_action`"] = function()
    ...
    child.ensure_normal_mode()
    child.lua(
        "vim.b.minicompletion_config = { fallback_action = function() _G.inside_local_fallback = true end }"
    )
    type_keys("i", "a")
    sleep(test_times.completion + 1)
...
T["Manual completion"]['applies `additionalTextEdits` from "textDocument/completion"'] = function()
    local validate = function(confirm_key)
        ...
        eq(
            get_lines(),
            {
                "from months.completion import September",
                "September" .. (is_explicit_confirm and "" or confirm_key),
            }
        )
...
T["Signature help"]["respects `config.window_dimensions.signature`"] = function()
    ...
    child.ensure_normal_mode()
    set_lines({ "" })
    child.b.minicompletion_config =
        { window_dimensions = { signature = { height = 10, width = 20 } } }
