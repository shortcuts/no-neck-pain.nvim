...
T["setup()"]["validates `config` argument"] = function()
    ...
    expect_config_error(
        { mappings = { object_scope_with_border = 1 } },
    ...
    expect_config_error(
        { options = { indent_at_cursor = 1 } },
        "options.indent_at_cursor",
        "boolean"
    )
...
T["get_scope()"]["respects `opts.try_as_border`"] = function()
    ...
    set_cursor(2, 1)
    eq(
        get_cursor_scope({ border = "both", try_as_border = true }).body,
        { top = 3, bottom = 3, indent = 2 }
    )
end
...
T["gen_animation()"]["respects `easing` argument"] = function()
    ...
    expect.error(function()
        child.lua([[MiniIndentscope.gen_animation('a')]])
    end, "one of")
end
...
T["gen_animation()"]["respects `opts` argument"] = function()
    ...
    -- Validates incorrect value
    expect.error(function()
        child.lua([[MiniIndentscope.gen_animation('linear', { unit = 'a' })]])
    end, "one of")
end
...
T["draw()"]["respects `config.draw.animation`"] = function()
    ...
    local command =
        string.format("MiniIndentscope.config.draw.animation = function() return %d end", duration)
    child.lua(command)
    validate(duration)
...
T["Auto drawing"]["respects ModeChanged event"] = function()
    ...
    child.lua(
        [[require('mini.indentscope').setup({ draw = { delay = 0, animation = function() return 0 end } })]]
    )
