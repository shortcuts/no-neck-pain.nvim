...
T["setup()"]["creates side effects"] = function()
    ...
    reload_module()
    expect.match(
        child.cmd_capture("hi MiniJump2dSpot"),
        "gui=bold,nocombine guifg=[Ww]hite guibg=[Bb]lack"
    )
    ...
    expect.match(
        child.cmd_capture("hi MiniJump2dSpot"),
        "gui=bold,nocombine guifg=[Bb]lack guibg=[Ww]hite"
    )
end
...
T["setup()"]["validates `config` argument"] = function()
    ...
    expect_config_error(
        { allowed_lines = { cursor_before = 1 } },
        "allowed_lines.cursor_before",
        "boolean"
    )
    expect_config_error({ allowed_lines = { cursor_at = 1 } }, "allowed_lines.cursor_at", "boolean")
    expect_config_error(
        { allowed_lines = { cursor_after = 1 } },
        "allowed_lines.cursor_after",
        "boolean"
    )
    ...
    expect_config_error(
        { allowed_windows = { current = "a" } },
        "allowed_windows.current",
        "boolean"
    )
    expect_config_error(
        { allowed_windows = { not_current = "a" } },
        "allowed_windows.not_current",
        "boolean"
    )
...
T["start()"]["respects `allowed_windows`"] = new_set({
    parametrize = {
        { { current = false } },
        { { not_current = false } },
        { { current = false, not_current = false } },
    },
}, {
