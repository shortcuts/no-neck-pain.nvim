...
local map_open = function(opts)
    child.lua("MiniMap.open(...)", { opts })
end

local map_refresh = function(opts, parts)
    child.lua("MiniMap.refresh(...)", { opts, parts })
end

local map_close = function()
    child.lua("MiniMap.close()")
end

-- Helpers related to 'mini.map'
local get_resolution_test_file = function(id)
    return "tests/dir-map/resolution_" .. id
end

local get_map_win_id = function()
    return child.lua_get("MiniMap.current.win_data[vim.api.nvim_get_current_tabpage()]")
end

local get_map_win_side = function()
    local win_config = child.api.nvim_win_get_config(get_map_win_id())
    if win_config.anchor == "NE" and win_config.col == child.o.columns then
        return "right"
    end
    if win_config.anchor == "NW" and win_config.col == 0 then
        return "left"
    end
    return "something is wrong"
end

local get_current = function()
    return child.lua_get("MiniMap.current")
end

local get_map_width = function()
    return child.api.nvim_win_get_width(get_map_win_id())
end
...
local mock_diagnostic = function()
    child.cmd("source tests/dir-map/mock-diagnostic.lua")
end

local mock_gitsigns = function()
    child.cmd("set rtp+=tests/dir-map")
end

local source_test_integration = function()
    child.cmd("source tests/dir-map/src-test-integration.lua")
end
...
local source_test_encode_symbols = function()
    child.lua(
        [[_G.test_encode_symbols = { '1', '2', '3', '4', resolution = { row = 1, col = 2 } }]]
    )
end
...
local get_n_shown_windows = function()
...
T["setup()"]["creates `config` field"] = function()
    ...
    local expect_config = function(field, value)
        eq(child.lua_get("MiniMap.config." .. field), value)
    end
...
T["setup()"]["validates `config` argument"] = function()
    ...
    local expect_all_encode_symbols_check = function()
        local expect_bad_config = function(err_pattern)
            expect.error(function()
                child.lua([[MiniMap.setup(_G.bad_config)]])
            end, err_pattern)
        end

        child.lua(
            "_G.bad_config = { symbols = { encode = { resolution = { col = 2, row = 2 } } } }"
        )
        for i = 1, 4 do
    ...
    expect_config_error(
        { symbols = { encode = { resolution = "a" } } },
        "symbols.encode.resolution",
        "table"
    )
    expect_config_error(
    ...
    expect_config_error(
        { window = { show_integration_count = 1 } },
        "window.show_integration_count",
        "boolean"
    )
...
local encode_strings = function(strings, opts)
    local cmd =
        string.format("MiniMap.encode_strings(%s, %s)", vim.inspect(strings), vim.inspect(opts))
    return child.lua_get(cmd)
end

T["encode_strings()"] = new_set()

T["encode_strings()"]["works"] = function()
...
T["encode_strings()"]["respects `strings` argument"] = function()
...
T["encode_strings()"]["respects `opts.n_rows`"] = function()
    ...
    eq(
        encode_strings({ "a", " ", " ", " ", " ", "a", "a", "a", " ", " ", " " }, { n_rows = 2 }),
        { "🬐", "🬀" }
    )
end
...
T["encode_strings()"]["respects `opts.symbols`"] = function()
    ...
    eq(
        child.lua_get(
            [[MiniMap.encode_strings({ '  aa', 'a  a' }, { symbols = _G.test_encode_symbols })]]
        ),
        { "14", "23" }
    )
end
...
T["encode_strings()"]["works with multibyte strings"] = function()
    eq(
        encode_strings({ "ыыыыыы", "ыыыы", "ыы", "aaaaaa", "aaaa", "aa" }),
        { "█🬎🬂", "█🬎🬂" }
    )
end
...
T["encode_strings()"]["can work with input dimensions being not multiple of resolution"] =
    function()
        eq(encode_strings({ "a" }), { "🬀" })
...
T["open()"]["sets important map buffer options"] = function()
    ...
    local validate_option = function(name, value)
        eq(child.api.nvim_buf_get_option(map_buf_id, name), value)
    end
...
T["open()"]["sets important window options"] = function()
    ...
    local validate_option = function(name, value)
        eq(child.api.nvim_win_get_option(map_win_id, name), value)
    end
...
T["open()"]["correctly computes window config"] = function()
    ...
    eq(
        child.api.nvim_win_get_option(win_id, "winblend"),
        child.lua_get("MiniMap.config.window.winblend")
    )
end
...
T["refresh()"]["works without opened map"] = function()
    expect.no_error(map_refresh)
end
...
T["close()"]["resets `MiniMap.current.opts` after closing last map window"] = function()
    local is_current_opts_reset = function()
        return child.lua_get("vim.tbl_count(MiniMap.current.opts)") == 0
    end
...
local map_open_with_integration = function(integration)
    local cmd = string.format(
        "MiniMap.open({ integrations = { MiniMap.gen_integration.%s() } })",
        integration
    )
    child.lua(cmd)
end
...
T["Window"]["fully updates on mode change to Normal"] = function()
    if child.fn.exists("##ModeChanged") == 0 then
        return
    end
...
T["Cursor in map window"]["moves cursor in source window"] = function()
    local init_win_id = child.api.nvim_get_current_win()
    local validate_source_cursor = function(ref_cursor)
        eq(child.api.nvim_win_get_cursor(init_win_id), ref_cursor)
    end
    set_lines(example_lines)
    set_cursor(2, 10)
    map_open()
