...
local get_active_items_names = function(buf_id)
    ...
    local items =
        child.lua_get("MiniStarter.content_to_items(MiniStarter.get_content(...))", { buf_id })
...
local mock_item = function(name, section)
    return {
        name = name,
        action = ("lua _G.item_name = %s"):format(vim.inspect(name)),
        section = section,
    }
end

local mock_itemstring = function(name, section)
    return ([[{ name = '%s', action = 'lua _G.item_name = "%s"', section = '%s' }]]):format(
        name,
        name,
        section
    )
...
T["Default content"]["computes `header` depending on time of day"] = new_set({
    ...
    test = function(hour)
        local cmd = ([[vim.fn.strftime = function(x) return x == '%%H' and '%s' or '' end]]):format(
            hour
        )
        child.lua(cmd)
        child.lua("MiniStarter.open()")
...
T["get_content()"]["works"] = function()
    ...
    eq(
        content[4],
        { { hl = "MiniStarterItem", item = content[4][1].item, string = "a", type = "item" } }
    )
    ...
    eq(
        { name = content_item.name, action = content_item.action, section = content_item.section },
        item
    )
end
...
T["gen_hook"]["adding_bullet() respects `place_cursor` argument"] = function()
    local reload = function(place_cursor)
        reload_from_strconfig({
            ...
            content_hooks = ([[{ MiniStarter.gen_hook.adding_bullet('aaa', %s) }]]):format(
                place_cursor
            ),
        })
    end
...
T["gen_hook"]["aligning()"]["respects arguments"] = new_set({
    ...
    test = function(args)
        reload_from_strconfig({
            ...
            footer = [['']],
            items = ("{ %s, %s }"):format(
                mock_itemstring("aaa", "AAA"),
                mock_itemstring("bbb", "AAA")
            ),
        })
        child.lua("MiniStarter.open()")
        child.expect_screenshot()
...
T["set_query()"]["does not allow query resulting in no active items"] = function()
    ...
    expect.match(
        get_latest_message(),
        '%(mini%.starter%) Query "c" results into no active items%. Current query: a'
    )
end
...
T["Cursor positioning"]["works with bullets"] = new_set({
    ...
    test = function(place_cursor, cursor_start, cursor_finish)
        reload_from_strconfig({
            items = example_itemstring,
            content_hooks = ("{ MiniStarter.gen_hook.adding_bullet(nil, %s) }"):format(
                place_cursor
            ),
            header = [['']],
            footer = [['']],
