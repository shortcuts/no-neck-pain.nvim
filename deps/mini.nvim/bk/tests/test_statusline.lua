...
local get_two_windows = function()
    ...
    return {
        active = cur_win,
        inactive = cur_win == wins_tabpage[1] and wins_tabpage[2] or wins_tabpage[1],
    }
end
...
T["setup()"]["sets proper autocommands"] = function()
    local validate = function(win_id, field)
        eq(
            child.api.nvim_win_get_option(win_id, "statusline"),
            "%!v:lua.MiniStatusline." .. field .. "()"
        )
    end
...
T["combine_groups()"]["allows empty `strings` to start new highlight"] = function()
    ...
    eq(
        combine_groups({ { hl = "AA" }, { hl = "BB", strings = { "b1", "b2" } } }),
        "%#AA#%#BB# b1 b2 "
    )
    eq(
        combine_groups({ { strings = { "a1", "a2" } }, { hl = "BB" }, { strings = { "c1" } } }),
        " a1 a2 %#BB# c1 "
    )
end
...
T["active()/inactive()"]["respects `config.content`"] = function(field)
    ...
    command = string.format(
        [[vim.b.ministatusline_config = { content = { %s = function() return 'bbb' end } }]],
        field
    )
    child.lua(command)
...
T["section_diagnostics()"]["respects `args.trunc_width`"] = function()
    set_width(100)
    eq(
        child.lua_get("MiniStatusline.section_diagnostics({ trunc_width = 100 })"),
        " E4 W3 I2 H1"
    )
    set_width(99)
...
T["section_fileinfo()"]["correctly asks 'nvim-web-devicons' for icon"] = function()
    child.cmd("e tmp.txt")
    eq(
        child.lua_get("_G.devicons_args"),
        { filename = "tmp.txt", extension = "txt", options = { default = true } }
    )
end
...
T["section_location()"]["respects `args.trunc_width`"] = function()
    set_width(100)
    eq(
        child.lua_get("MiniStatusline.section_location({ trunc_width = 100 })"),
        '%l|%L│%2v|%-2{virtcol("$") - 1}'
    )
    set_width(99)
...
T["Default content"]["inactive"] = function()
    ...
    eq(
        child.api.nvim_win_get_option(wins.inactive, "statusline"),
        "%!v:lua.MiniStatusline.inactive()"
    )
