...
-- Custom validators
local validate_edit =
    function(before_lines, before_cursor, after_lines, after_cursor, test_action, ...)
...
local validate_edit1d =
    function(before_line, before_column, after_line, after_column, test_action, ...)
        validate_edit(
            { before_line },
            { 1, before_column },
            { after_line },
...
local mock_treesitter_builtin = function()
    child.cmd("source tests/dir-surround/mock-lua-treesitter.lua")
end

local mock_treesitter_plugin = function()
    child.cmd("set rtp+=tests/dir-surround")
end
...
T["setup()"]["properly handles `config.mappings`"] = function()
    local has_map = function(lhs, rhs)
        return child.cmd_capture("nmap " .. lhs):find(rhs or "MiniSurround") ~= nil
    end
...
T["gen_spec"]["input"]["treesitter()"]["validates `captures` argument"] = function()
    ...
    local validate = function(args)
        expect.error(function()
            child.lua([[MiniSurround.gen_spec.input.treesitter(...)]], { args })
        end, "captures")
    end
...
T["gen_spec"]["input"]["treesitter()"]["validates builtin treesitter presence"] = function()
    ...
    expect.error(function()
        type_keys("sd", "F", "<CR>")
    end, vim.pesc([[(mini.surround) Can not get query for buffer 1 and language 'lua'.]]))

    -- Parser
    child.bo.filetype = "aaa"
    expect.error(function()
        type_keys("sd", "F", "<CR>")
    end, vim.pesc([[(mini.surround) Can not get parser for buffer 1 and language 'aaa'.]]))
end
...
T["Add surrounding"]["validates single character user input"] = function()
    ...
    eq(
        get_latest_message(),
        "(mini.surround) Input must be single character: alphanumeric, punctuation, or space."
    )
end
...
T["Add surrounding"]["respects `vim.b.minisurround_config`"] = function()
    child.b.minisurround_config =
        { custom_surroundings = { ["<"] = { output = { left = ">", right = "<" } } } }
...
local set_custom_surr = function(tbl)
    child.lua("MiniSurround.config.custom_surroundings = " .. vim.inspect(tbl))
end
...
T["Custom surrounding"]["documented examples"]["function call with name from user input"] =
    function()
        child.lua([[MiniSurround.config.search_method = 'cover_or_next']])
...
T["Custom surrounding"]["documented examples"]["balanced parenthesis with big enough width"] =
    function()
    ...
    '^.().*().$'
        ...
        child.lua(
            "MiniSurround.config.custom_surroundings = { p = { input = _G.wide_parens_spec } }"
        )
        child.lua([[MiniSurround.config.search_method = 'cover_or_next']])
...
T["Custom surrounding"]["documented examples"]["handles function as specification item"] =
    function()
        child.lua([[_G.c_spec = {
    '%b()',
    function(s, init) if init > 1 then return end; return 2, s:len() end,
