...
T["make_tabline_string()"]["attaches correct highlight group to unnamed buffers"] = function()
    ...
    eq(
        eval_tabline(true),
        "%#MiniTablineModifiedCurrent# * %#MiniTablineHidden# !(2) %#MiniTablineFill#"
    )
end
...
T["make_tabline_string()"]["respects `config.show_icons`"] = function()
    ...
    eq(
        eval_tabline(true),
        "%#MiniTablineHidden#  LICENSE %#MiniTablineCurrent#  aaa.lua %#MiniTablineFill#"
    )
    ...
    eq(
        eval_tabline(true),
        "%#MiniTablineCurrent# LICENSE %#MiniTablineHidden# aaa.lua %#MiniTablineFill#"
    )
...
T["make_tabline_string()"]["deduplicates named labels"] = function()
    ...
    eq(
        eval_tabline(),
        (" dir1/aaa  dir2/aaa  dir1/dir_nested/aaa  dir2/dir_nested/aaa  %s/aaa "):format(
            cur_dir_basename
        )
    )
end
