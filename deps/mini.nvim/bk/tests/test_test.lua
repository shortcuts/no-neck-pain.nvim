...
local get_current_all_cases = function()
    ...
    res = vim.tbl_map(function(case)
        case.hooks = {
            pre = vim.tbl_map(loadstring, case.hooks.pre),
            post = vim.tbl_map(loadstring, case.hooks.post),
        }
...
local testrun_ref_file = function(name)
    local find_files_command =
        string.format([[_G.find_files = function() return { '%s' } end]], get_ref_path(name))
    child.lua(find_files_command)
    child.lua(
        "MiniTest.run({ collect = { find_files = _G.find_files }, execute = { reporter = {} } })"
    )
...
T["run()"]["handles `parametrize`"] = function()
    ...
    local short_res = vim.tbl_map(function(c)
        ...
        return {
            args = c.args,
            desc = desc,
            passed_args = c.exec.fails[1]:match("Passed arguments: (.-)\n  Traceback.*$"),
        }
    end, res)

    eq(
        short_res[1],
        { args = { "a" }, desc = { "parametrize", "first level" }, passed_args = '"a"' }
    )
    eq(
        short_res[2],
        { args = { "b" }, desc = { "parametrize", "first level" }, passed_args = '"b"' }
    )

    eq(
        short_res[3],
        { args = { "a", 1 }, desc = { "parametrize", "nested", "test" }, passed_args = '"a", 1' }
    )
    eq(
        short_res[4],
        { args = { "a", 2 }, desc = { "parametrize", "nested", "test" }, passed_args = '"a", 2' }
    )
    eq(
        short_res[5],
        { args = { "b", 1 }, desc = { "parametrize", "nested", "test" }, passed_args = '"b", 1' }
    )
    eq(
        short_res[6],
        { args = { "b", 2 }, desc = { "parametrize", "nested", "test" }, passed_args = '"b", 2' }
    )
...
T["run()"]["handles same function in `*_once` hooks"] = function()
    ...
    local fails = vim.tbl_map(function(x)
        return x:gsub("\n  Traceback.*$", "")
    end, case.exec.fails)
    eq(
        fails,
        { "Same function", "Same function", "Same hook test", "Same function", "Same function" }
    )
end
...
T["run()"]["appends traceback to fails"] = function()
    ...
    if n == 0 then
        error("No actual fails was tested", 0)
    end
end
...
T["run_at_location()"]["works with non-default input"] = new_set(
    { parametrize = { { 3 }, { 4 }, { 5 } } },
    {
        function(line)
            local path = get_ref_path("testref_run.lua")
            local command =
                string.format([[MiniTest.run_at_location({ file = '%s', line = %s })]], path, line)
            child.lua(command)
...
T["collect()"]["respects `emulate_busted` option"] = function()
    ...
    local command = string.format(
        [[MiniTest.collect({ find_files = function() return { '%s' } end })]],
        busted_file
    )
    expect.error(function()
        child.lua(command)
    end, "attempt to call global")
end
...
T["collect()"]["respects `filter_cases` option"] = function()
    ...
    child.lua(
        [[vim.b.minitest_config = { collect = { filter_cases = function() return false end } }]]
    )
    child.lua("_G.cases = MiniTest.collect()")
    eq(child.lua_get("#_G.cases"), 0)
end
...
T["execute()"]["respects `reporter` option"]["partial"] = function()
    ...
    child.lua(
        "vim.b.minitest_config = { execute = { reporter = { update = function() _G.was_in_update = true end } } }"
    )
...
T["stop()"]["works"] = function()
    ...
    child.lua(
        "MiniTest.execute(_G.cases, { reporter = { start = function() MiniTest.stop() end } })"
    )
...
T["expect"]["error()"]["works"] = function()
    ...
    end, 'error matching pattern "bb"%..*Observed error:')
end

T["expect"]["error()"]["respects `pattern` argument"] = function()
    expect.error(function()
        MiniTest.expect.error(error, 1)
    end, "pattern.*expected string")

    -- `nil` and `''` are placeholders for 'any error'
    expect.no_error(function()
        MiniTest.expect.error(error, "")
    end)
    expect.no_error(function()
        MiniTest.expect.error(error, nil)
    end)
end
...
T["expect"]["reference_screenshot()"]["locates problem"] = function()
    ...
    validate(
        screen_text_columns,
        "Different number of columns in `text` line 1%. Reference: 12%. Observed: 11%."
    )
    ...
    validate(
        screen_attr_columns,
        "Different number of columns in `attr` line 1%. Reference: 12%. Observed: 11%."
    )
    ...
    validate(
        screen_text_cell,
        'Different `text` cell at line 1 column 2%. Reference: "a"%. Observed: "X"%.'
    )
    ...
    validate(
        screen_attr_cell,
        'Different `attr` cell at line 1 column 2%. Reference: "0"%. Observed: "X"%.'
    )
end
...
T["expect"]["reference_screenshot()"]["correctly infers reference path"] = function()
    ...
    eq(
        MiniTest.expect.reference_screenshot(
            ...
            "tests/dir-test/intermediate-screenshot"
        ),
        true
    )
...
local suffix = useful_punctuation
    .. linux_forbidden
    .. windows_forbidden
    .. whitespace
    .. special_characters
...
T["expect"]["reference_screenshot()"]["correctly sanitizes path for Windows "] =
    validate_path_sanitize
T["expect"]["reference_screenshot()"]["correctly sanitizes path for Windows #2."] =
    validate_path_sanitize
...
T["expect"]["reference_screenshot()"]["creates refernce if it does not exist"] = function()
    ...
    eq(
        MiniTest.current.case.exec.notes,
        { "Created reference screenshot at path " .. vim.inspect(path) }
    )
...
T["child"]["api"] = function()
    local method = function()
        return child.api.nvim_buf_set_lines(0, 0, -1, true, { "aaa" })
    end
...
T["child"]["api_notify"] = function()
    local method = function()
        return child.api_notify.nvim_buf_set_lines(0, 0, -1, true, { "aaa" })
    end
    validate_child_method(method, { prevent_hanging = false })
end
...
T["child"]["redirected method tables"]["method"] = function(tbl_name, field_name, args)
    ...
    local method = function()
        return child[tbl_name][field_name](unpack(args))
    end
...
T["child"]["scoped variables"] =
    new_set({ parametrize = { { "g" }, { "b" }, { "w" }, { "t" }, { "v" }, { "env" } } })

T["child"]["scoped variables"]["method"] = function(scope)
    local method = function()
        return child[scope].char
    end
...
T["child"]["scoped options"]["method"] = function(tbl_name, field_name, _)
    local method = function()
        return child[tbl_name][field_name]
    end
...
T["child"]["cmd_capture()"] = function()
    local method = function()
        return child.cmd_capture([[echomsg 'Hello world']])
    end
...
T["child"]["lua_notify()"] = function()
    local method = function()
        return child.lua_notify("_G.n = 0")
    end
...
T["child"]["ensure_normal_mode()"]["works"] = new_set(
    { parametrize = { { "i" }, { "v" }, { ":" }, { "R" } } },
    {
        function(keys)
...
T["gen_reporter"]["buffer"] = new_set({
    ...
    test = function(opts_element)
        ...
        local command =
            string.format("_G.reporter = MiniTest.gen_reporter.buffer({ %s })", opts_element)
        child.lua(command)
        child.lua(
            ([[MiniTest.run_file('%s', { execute = { reporter = _G.reporter } })]]):format(path)
        )
...
T["gen_reporter"]["stdout"] = new_set({
    ...
    test = function(env_var)
        ...
        child.fn.termopen(
            env_var
                .. [[ nvim --headless --clean -n -u 'tests/dir-test/init_stdout-reporter_works.lua']]
        )
