...
---
---@param name string: the name/filetype of the integration.
---@return string?: the position ("left", "right", or "none"), or nil if not found.
---@private
function ui.get_integration_position(name)
    if _G.NoNeckPain.config.integrations[name] ~= nil then
        return _G.NoNeckPain.config.integrations[name].position
    end
